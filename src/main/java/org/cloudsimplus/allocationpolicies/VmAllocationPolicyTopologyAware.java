...
public class VmAllocationPolicyTopologyAware extends VmAllocationPolicyAbstract {
    ...
    /**
     * Weight applied to the always-on {@code LeastAllocated} contribution
     * (fraction of CPU PEs and RAM reserved by placed VMs). Small by design:
     * acts as a Kubernetes-style tie-breaker when the primary policy score
     * (cost / latency / spread) is equal across candidates, without
     * overriding genuine differences in the primary policy.
     *
     * <p>Mirrors {@code NodeResourcesLeastAllocated} in Kubernetes' default
     * scheduler, which prefers less-utilized nodes among otherwise equal
     * candidates.</p>
     */
    public static final double LEAST_ALLOCATED_WEIGHT = 0.01;
    ...
    protected double score(final Vm vm, final Host host) {
        final double policyScore = switch (policy) {
    ...
    /**
     * Fraction of host resources already reserved by placed VMs, averaged over
     * CPU PEs and RAM. Lower means less-loaded means better fit. Mirrors
     * Kubernetes' {@code NodeResourcesLeastAllocated} priority function:
     * given two otherwise-equal candidates, prefer the one with more spare
     * capacity. Always-on (composed into every policy's score).
     */
    private double leastAllocatedScore(final Host host) {
        final long peTotal = host.getPesNumber();
        final long peAlloc = host.getVmList().stream().mapToLong(Vm::getPesNumber).sum();
        final double cpuFrac = peTotal == 0 ? 0.0 : (double) peAlloc / peTotal;

        final long ramCap = host.getRam().getCapacity();
        final long ramAlloc = host.getRam().getAllocatedResource();
        final double ramFrac = ramCap == 0 ? 0.0 : (double) ramAlloc / ramCap;

        return (cpuFrac + ramFrac) / 2.0;
    }
